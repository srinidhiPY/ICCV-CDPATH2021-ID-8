"""
Finetuning Self-Supervised model on MHIST dataset with Curriculum - II (i.e., hard-to-very-hard) stage
"""
import argparse
import os
import time
...
def train(args, model, classifier, train_loader, optimizer, epoch):
    ...
    for batch_idx, (input, target) in enumerate(tqdm(train_loader, disable=False)):
        ...
        total_sum_hard_samples_k = sum(hard_samples_top_k2)

        # Select top_k and top_k' output and target values
...
def parse_args():
    ...
    parser = argparse.ArgumentParser('Finetuning Self-Supervised model on MHIST dataset with Curriculum - II (i.e., hard-to-very-hard) stage')
...
##########
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30], gamma=0.95)
