"""
Finetuning Self-Supervised model on Camelyon16 dataset with Curriculum - II (i.e., hard-to-very-hard) stage
"""
import argparse
import os
import time
...
from dataset import DatasetTBLN_Supervised_train, DatasetTBLN_eval
...
def parse_args():
    ...
    parser = argparse.ArgumentParser('Finetuning Self-Supervised model on Camelyon16 dataset with Curriculum - II (i.e., hard-to-very-hard) stage')
    ...
    parser.add_argument('--num_epoch', type=int, default=60, help='epochs to train for.')
    ...
    parser.add_argument('--model_path', type=str,
                        default='./models/cam_curriculum_I_finetuned_model.pt',
                        help='path to load curriculum-I fine-tuned model')
    ...
    # Data paths
    parser.add_argument('--train_image_pth', default='./CAMELYON/Fine_Tuning/train/')
    parser.add_argument('--val_image_pth',
                        default='./CAMELYON/Fine_Tuning/val/')
    parser.add_argument('--labeled_train', default=1.0, type=float, help='portion of the train data with labels - 1(full), 0.1/0.25/0.5')


    # Parameters
    parser.add_argument('--image_size', default=256, type=int, help='patch size width 256')
    parser.add_argument('--alpha', default=0.1, type=int, help='portion of hard samples in each mini-batch to select top-K hard examples for curriculum-II fine-tuning')
...
def main():
    ...
    if args.mode == 'fine-tuning':
        ...
        # Train set
        train_dataset = DatasetTBLN_Supervised_train(args.train_image_pth, args.image_size)

        # Validation Set
        val_dataset = DatasetTBLN_eval(args.val_image_pth, args.image_size)

        # train and validation split
        train_dataset_idx = list(range(len(train_dataset.datalist)))
        val_dataset_idx = list(range(len(val_dataset.datalist)))
        print('total number of train samples in the dataset', len(train_dataset_idx))
        print('total number of val samples in the dataset', len(val_dataset_idx))

        #### Limited fine-tuning Split (10, 25, 50, 100)
        train_idx = np.random.choice(train_dataset_idx, int(args.labeled_train * len(train_dataset_idx)))
        print('subset of training samples selected for fine-tuning', len(train_idx))

        train_sampler = SubsetRandomSampler(train_idx)
        val_sampler = SubsetRandomSampler(val_dataset_idx)
